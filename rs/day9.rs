...
fn proc_inst<'a>(
    inst: &'a Instruction,
    positions: &'a Vec<Coord>,
    visited: &'a mut HashSet<Coord>,
) -> Vec<Coord> {
    // println!("Inst: {:?}", inst);
    let (dir, num) = inst;
    ...
    for _ in 0..*num {
        ...
        positions_tmp[1..]
            .iter()
            .cloned()
            .fold(new_positions[0], |prev, next| {
...
fn main() -> Result<(), std::io::Error> {
    ...
    println!(
        "Number of visited coordinates with two knots: {:?}",
        visited.len()
    );
    ...
    println!(
        "Number of visited coordinates with ten knots: {:?}",
        visited_part2.len()
    );
    Ok(())
}
